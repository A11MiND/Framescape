  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-3xl px-6 py-8">
        <h1 className="mb-6 text-lg font-medium">积分</h1>

        <div className="mb-8 grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
            <p className="text-xs uppercase tracking-wide text-zinc-500">可用余额</p>
            <p className="mt-1 font-mono text-2xl text-violet-300">
              <AnimatedNumber value={balance.data?.balance ?? 0} />
            </p>
          </div>
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
            <p className="text-xs uppercase tracking-wide text-zinc-500">持有中（作业进行中预扣）</p>
        ...
        <p className="mb-3 text-xs uppercase tracking-wide text-zinc-500">流水</p>
        <div className="overflow-hidden rounded-xl border border-zinc-800">
          ...
          {ledger.isSuccess && entries.length === 0 && (
            <p className="p-4 text-center text-sm text-zinc-500">还没有流水记录</p>
          )}
        </div>
        ...
        {ledger.hasNextPage && (
          ...
          >
            {ledger.isFetchingNextPage ? '加载中…' : '加载更多'}
          </button>
        )}
      </div>
    </AppShell>
...
function LedgerRow({ entry }: { entry: CreditLedgerEntry }) {
  ...
  return (
    <tr className="border-b border-zinc-800 last:border-0">
      <td className="px-4 py-2.5 text-zinc-400">{DIRECTION_LABEL[entry.direction] ?? entry.direction}</td>
      ...
      <td className="px-4 py-2.5 text-right font-mono text-xs text-zinc-600">
        {new Date(entry.created_at).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}
      </td>
