...
func (e *Engine) startLoopController(ctx context.Context, workflowRunID string, wf *model.Workflow, loopTR *store.TaskRun, resolvedInputs *model.Inputs) error {
	...
	//
	// PATCHED (aigc-platform, see docs/aether-validation-report.md's W6
	// addendum): the original code stopped here after UpdateTaskRun, which
	// leaves two problems for any real DAG depending on this loop:
	//
	//  1. No parent-scope advancement. A normal (non-empty) loop's
	//     completion is driven by advanceScope walking up from the *last
	//     iteration's* parent (the loop itself) once allTerminal() sees all
	//     iteration TaskRuns are terminal — see engine_sched.go's
	//     "7. All children are terminal ... walk up" step. A zero-iteration
	//     loop never creates any iteration TaskRuns, so nothing ever fires
	//     that walk-up, and the loop's own parent DAG never learns the loop
	//     finished: it stays Created/Running forever, and the whole
	//     workflow deadlocks. Reproduced directly against this vendored
	//     copy with a 2-task DAG (docs/aether-validation-report.md's
	//     "empty Loop" test): the parent DAG's phase never left "Created"
	//     even though the (zero-item) Loop itself showed "Succeeded".
	//  2. No Outputs. Downstream tasks that reference
	//     tasks.<loop>.outputs.parameters.* via valueFrom would bind
	//     against a nil Outputs — confirmed directly: with only fix #1
	//     applied, a downstream task consuming a zero-iteration "list"
	//     loop's output failed to bind with "unexpected end of JSON input"
	//     (an empty valueFrom resolution isn't even valid JSON `null`).
	//     internal.AggregateResults(nil, ...) intentionally returns nil
	//     outputs for zero results (internal/loop_test.go's
	//     TestAggregateResults_EmptyReturnsSucceeded) — reasonable for
	//     "first"/"last" strategies, where zero iterations truly has no
	//     candidate value, but useless for "list" strategy, whose whole
	//     purpose is collecting a variable-length (possibly zero-length)
	//     result set: the natural, usable value for zero iterations is an
	//     empty array per declared parameter, not "no output at all".
	//     Synthesized below directly from the declared parameter names
	//     (Aggregate.Parameters, falling back to the loop's own declared
	//     Outputs.Parameters) since there are no iteration results to
	//     discover names from.
	//
	// The fix: build phase/outputs consistent with the normal path (via
	// AggregateResults, called with an empty result set, then widened for
	// "list" strategy per #2 above) and continue the walk up via
	// advanceScope(loopTR.ParentRunID) exactly as step 7 of advanceScope's
	// finalization does ("parentRunID = parentTR.ParentRunID").
	if len(iterations) == 0 {
		phase, _, aggregated := internal.AggregateResults(nil, loop.Aggregate)
		noIterMsg := "loop had no iterations"
		var outputs *model.Outputs
		if aggregated != nil {
			outputs = &model.Outputs{Phase: phase, ExecOutputs: model.ExecOutputs{Parameters: aggregated.Parameters}}
		} else if loop.Aggregate != nil && loop.Aggregate.Strategy == model.AggregateStrategyList {
			names := loop.Aggregate.Parameters
			if len(names) == 0 && loop.Outputs != nil {
				for _, p := range loop.Outputs.Parameters {
					names = append(names, p.Name)
				}
			}
			params := make([]model.Parameter, 0, len(names))
			for _, name := range names {
				params = append(params, model.Parameter{Name: name, Value: []byte("[]")})
			}
			if len(params) > 0 {
				outputs = &model.Outputs{Phase: phase, ExecOutputs: model.ExecOutputs{Parameters: params}}
			}
		}
		if _, updateErr := e.store.UpdateTaskRun(ctx, &store.TaskRun{
			RunID:   loopTR.RunID,
		...
		return e.advanceScope(ctx, workflowRunID, wf, loopTR.ParentRunID)
	}
