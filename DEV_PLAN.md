...
>
> **2026-08-19 更新**：v0.2 已被 `AIGC生成平台-PRD-v1.0.md` 取代（v0.2 文件本身已删除，git 历史可查）。上面这条"每项任务可追溯到 PRD 章节号"的约定，从 §0-§14（W0.5~W7）这些历史记录开始就**对不上了**——那些 `§5.4`/`F6.8` 式的引用指向的是 v0.2 的章节结构，v1.0 换了一套完全不同的组织方式（§0-§9，功能清单用 F1-F9 模块化重编号，不再逐条对应旧编号）。不逐条回填修正，只是提醒：往下读到旧编号引用时，去 git 历史翻 v0.2 原文，不要去 v1.0 里找同名章节。新增的 §15 是 v1.0 发布后追加的实际进度记录。
...
## 15. W7 之后的增量迭代（本节起不再逐条对应 v0.2 旧章节号）

W7 收口之后，产品又长了 33 个提交（`18dc114`..`HEAD`）没有被本文档追踪过——这些不是 W1-W7 计划内的任务，是收口之后陆续加的功能/修复。这里第一次把它们记录下来，按主题分组；**2026-08-19 之前的条目**只对照 commit message 整理，不是由记录这次更新的会话亲自验证过的，跟本文档 W1-W7 部分"逐条真实调用验证"的严谨程度不是一回事，标注清楚以免误读成同等可信度。

**社区**
- 发布/浏览社区作品（`fd1b008`）
- "我发布的"自筛选 tab + 撤销发布 + feed 对游客开放（`e0b6960`，2026-08-19 会话内完成，真实验证过）
- 连续发布积分奖励：3/10/30 天里程碑（`3cb637a`，同上，真实验证过）

**生成能力**
- 连续图跨镜头引用（`1a874f5`）
- 连续影片支持视频类型的 r2va 锚点（`93bfa90`）
- 叙事连续性生成（连续图/连续影片/四格漫画）（`06fbce1`）
- image.single 与 image.batch 合并、四格漫画格数可变、作业加回收站（`87c202b`，2026-08-19 会话内完成，真实验证过）
- 连续影片"跳过预览"直接 2K 定稿（`54a9057`）
- 参考图支持从 image.single 扩展到全部形态（`0dc90e5`）

**资产/回收站**
- 回收站：软删 30 天自动清空（`ce808b8`）
- 修复软删除资产仍可被完整访问、两处"一直转圈"的死链接 bug（`ee3bad2`）

**账号与登录**
- Google 登录/手机号注册/邮箱验证码骨架（`8fbfeb9`，2026-08-19 会话内完成，真实验证过，密钥待配置）
- 首页匿名试用直接并入主生成按钮（`52b0eb0`）
- 登录页改版为左右分栏+生成作品展示（`3d08f8a`）
- 移除作业详情页 DAG 视图、隐藏角色 seed、修复积分流水多语言、加账号改密码（`ecf590c`）

**创作台交互**
- 角色绑定数量从最多 2 个放开（`8096d3d`）
- AI 提示词改写按钮（`8924b19`）
- `#` 提及与 AI 改写扩展到全部提示词输入框（`7112bba`）
- 连续图图标/连接线视觉修复（`55f4af0`）
- 去掉创作台预填示例文案、镜头徽章去术语化（`83f733a`）
- 格式卡片图标去 emoji 化、去掉原生下拉选择（`91ab503`）
- 点击外部关闭浮层（角色槽选择器/提及输入框/通知中心）（`e9f444c`）

**预设库**
- 支持从 `/预设` 页面直接创建预设（`b5162b2`）
- 预设加真实封面图、创作台选择器改版、库扩容（`2138793`）
- 放大预设封面、角色/画风多样化（`d9d5511`）
- 格式切换卡片移到创作台上方、轮播收窄（`3c051e4`）
- 预设封面主体多样化，不只是角色（`daa0bb7`）
- 14 个预置预设名称补英文（`dd4c04c`）
